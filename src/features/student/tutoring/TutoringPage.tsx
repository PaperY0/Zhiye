...
function PrimaryButton({ children, onClick, disabled = false }: { children: ReactNode; onClick?: () => void; disabled?: boolean }) {
  return <button className="min-h-14 rounded-2xl border border-[#bed7c1] bg-[#dceedd] px-7 font-black text-[#416449] shadow-lg shadow-[#527a5a]/10 transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-60" disabled={disabled} onClick={onClick} type="button">{children}</button>
}
...
export function TutoringPage() {
  ...
  return <div className="mx-auto w-full max-w-6xl space-y-5 px-4 py-5 sm:px-6 lg:px-8">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><div className="mb-2 flex items-center gap-2 text-sm font-bold text-[#54705b]"><Sparkles aria-hidden="true" size={18} /><PinyinText text="一步一步来" /></div><h1 className="text-3xl font-black tracking-tight text-[#102218] sm:text-4xl"><PinyinText text="拍照答疑" /></h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#64756a] sm:text-base">先确认题目，再告诉我你卡在哪里。我们从提示开始，保留你自己思考的空间。</p></div><StatusChip tone="info">题图仅发送至本地 OCR 服务</StatusChip></header>
    <ol aria-label="答疑进度" className="grid grid-cols-4 gap-2 text-center text-xs font-bold text-[#6d7d72]">{progressSteps.map((label, index) => <li aria-current={currentProgress === index ? "step" : undefined} className="space-y-2" key={label}><span className={`mx-auto grid size-8 place-items-center rounded-full shadow-sm ${index <= currentProgress ? "border border-[#bed7c1] bg-[#dceedd] text-[#416449]" : "bg-white/75"}`}>{index + 1}</span><PinyinText text={label} /></li>)}</ol>

    {state.step === "upload" ? <GlassSurface className="grid min-h-[420px] place-items-center rounded-[32px] p-6 text-center sm:p-10" weight="sheet"><div className="max-w-xl"><div className="mx-auto grid size-20 place-items-center rounded-[28px] bg-[#e5efe0] text-[#41694d]"><Camera aria-hidden="true" size={36} /></div><h2 className="mt-6 text-2xl font-black text-[#14251b]">把题目放进来</h2><p className="mt-3 text-sm leading-6 text-[#68786e]">请选择能看清题干、数字和解题痕迹的图片。识别后你可以修改题目文本。</p><label className="mt-7 inline-flex min-h-16 cursor-pointer items-center justify-center gap-3 rounded-2xl border border-[#bed7c1] bg-[#dceedd] px-7 py-4 font-black text-[#416449]"><ImagePlus aria-hidden="true" size={22} />选择题目图片<input accept="image/*" aria-label="选择题目图片" className="sr-only" onChange={selectFile} type="file" /></label></div></GlassSurface> : null}
    ...
    {state.step === "ocr-failed" ? <FlowCard><div className="text-center"><RefreshCw className="mx-auto text-[#8a651c]" size={36} /><h2 className="mt-5 text-2xl font-black text-[#14251b]">请补拍一张清晰题图</h2><p className="mt-3 text-[#68786e]" role="alert">{state.ocrError}</p><label className="mt-6 inline-flex min-h-14 cursor-pointer items-center justify-center rounded-2xl border border-[#bed7c1] bg-[#dceedd] px-7 font-black text-[#416449]">重新选择题目图片<input accept="image/*" aria-label="重新选择题目图片" className="sr-only" onChange={selectFile} type="file" /></label></div></FlowCard> : null}
    ...
    {state.step === "complete" ? <FlowCard><div className="text-center"><CheckCircle2 className="mx-auto text-[#477054]" size={38} /><h2 className="mt-5 text-2xl font-black text-[#14251b]">已经保存到错题本</h2><div className="mt-7 flex flex-wrap justify-center gap-3"><a className="inline-flex min-h-14 items-center gap-2 rounded-2xl border border-[#bed7c1] bg-[#dceedd] px-7 font-black text-[#416449]" href="#/student/mistakes">查看我的错题本<ArrowRight aria-hidden="true" size={18} /></a><button className="min-h-14 rounded-2xl border border-[#aab9aa] bg-white/55 px-6 font-black text-[#34513d]" onClick={() => { dispatch({ type: "RESET" }); setOcrText(""); setAttempt(""); setRetell(""); setTransferAnswer("") }} type="button">再问一道题</button></div></div></FlowCard> : null}
  </div>
}
